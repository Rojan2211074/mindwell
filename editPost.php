...
<?php if(isset($_POST['update'])){
    ...
    $nuploadlocation="uploads/posts/".$nimgname;
    ...
    if (!empty($nimgname)) {
        ...
        $update=$posts->editPost($pid,$ntitle,$ndescription,$noldphoto,$nuploadlocation,$nimgname,$ntmpname);
    } else {
        // File is not submitted, update other fields without file upload
        $posts->editPostNoFile($pid, $ntitle, $ndescription);}
    if($update){
...
<body>
  <main>
    ...
    while($row=mysqli_fetch_array($qry)){
        ...
        $ethumbimg=$row['featureimg'];
        $oldthumbimg=$row['featureimg'];
        
        $estatus=$row['status'];
    echo "<form action='' name='update' method='post' enctype='multipart/form-data'>";
    echo "<fieldset>";
    echo'<label for="description">Title</label>';
    echo "<br>";
    // echo "<input type=number name=uid required value=$eid readonly>";
    echo "<input type=text name=title required value='$etitle'>";
    echo "<br>";
    echo'<label for="description">Description</label>';
    echo "<br>";
    echo "<input type=text name=description required value='$edescription'>";
   
    echo "<br>";
    echo"<img src='uploads/posts/$ethumbimg' style='  object-fit: cover; height:100px; width:100px; border-radius:100%; margin:2rem;'>";
    echo "<br>";
